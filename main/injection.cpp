...
static float volToTurns(float cm3) { return cm3 * TURNS_PER_CM3_VOL; }

static float turnsToVol(float turns) {
  if (TURNS_PER_CM3_VOL == 0.0f)
    return 0.0f;
...
bool update(CanBus &motor) {
  ...
  if (stateEntry) {
...
#if APP_DEBUG
      ESP_LOGW("INJECT", "FILL_VOLUME_TOO_LARGE: req=%.2f max=%.2f",
               currentMould.fillVolume, maxInjectVolume);
#endif
      error = true;
      complete = true;
      stateEntry = false;
    ...
    if (torqueMode) {
      ...
      // Use GENERAL_MACHINE_CURRENT_LIMIT for headroom (raised to 60.0f in
      // config.h)
      MotorWrapper::setMotorLimits(motor, INJECT_FILL_CONTROLLER_VEL_LIMIT,
                                   GENERAL_MACHINE_CURRENT_LIMIT, MODULE_INJECT,
                                   "Inject Torque");
      if (!MotorWrapper::setModeAndMove(motor, 1, 6, currentMould.injectTorque,
                                        MODULE_INJECT, "Inject Torque")) {
        return false;
      }
    } else {
      ...
      float fillCurrent = currentMould.fillPressure > 0.0f
                              ? currentMould.fillPressure
                              : INJECT_FILL_CURRENT;
      float fillVel = currentMould.fillSpeed > 0.0f
                          ? currentMould.fillSpeed
                          : INJECT_FILL_TRAP_VEL_LIMIT;
      float fillAccel = currentMould.fillTrapAccel > 0.0f
                            ? currentMould.fillTrapAccel
                            : INJECT_FILL_ACCEL;
      float fillDecel = currentMould.fillTrapDecel > 0.0f
                            ? currentMould.fillTrapDecel
                            : INJECT_FILL_DECEL;

      MotorWrapper::setMotorLimits(motor, INJECT_FILL_CONTROLLER_VEL_LIMIT,
                                   fillCurrent, MODULE_INJECT, "Inject Fill");
      MotorWrapper::setTrapTrajParams(motor, fillVel, fillAccel, fillDecel,
                                      MODULE_INJECT, "Fill Traj");
      if (!MotorWrapper::setModeAndMove(motor, 3, 5, targetInjectPos,
                                        MODULE_INJECT, "Pos Inject")) {
        return false;
      }
    }
    lastCommandTime = now;
  ...
  if (phase == FILLING) {
    ...
    bool positionClose =
        std::fabs(bds.getPosition() - targetInjectPos) < INJECT_POS_TOLERANCE;
    ...
    if (velocityLow && positionClose) {
      ...
      float packCurrent = currentMould.packPressure > 0.0f
                              ? currentMould.packPressure
                              : INJECT_PACK_CURRENT;
      float packVel = currentMould.packSpeed > 0.0f
                          ? currentMould.packSpeed
                          : INJECT_PACK_TRAP_VEL_LIMIT;
      float packAccel = currentMould.packTrapAccel > 0.0f
                            ? currentMould.packTrapAccel
                            : INJECT_PACK_ACCEL;
      float packDecel = currentMould.packTrapDecel > 0.0f
                            ? currentMould.packTrapDecel
                            : INJECT_PACK_DECEL;

      MotorWrapper::setMotorLimits(motor, INJECT_PACK_CONTROLLER_VEL_LIMIT,
                                   packCurrent, MODULE_INJECT, "Inject Pack");
      MotorWrapper::setTrapTrajParams(motor, packVel, packAccel, packDecel,
                                      MODULE_INJECT, "Pack Traj");
      MotorWrapper::setModeAndMove(motor, 3, 5, targetPackPos, MODULE_INJECT,
                                   "Pos Pack");
      lastCommandTime = now;
      return false;
    }
    ...
    if (phaseElapsed > INJECT_FILL_TIMEOUT_MS) {
      error = true;
      complete = true;
      MotorWrapper::setModeAndMove(motor, 2, 1, 0, MODULE_INJECT,
                                   "Inject Timeout Stop");
      return true;
    }
    return false;
  ...
  if (phase == PACKING) {
    uint64_t packTimeMs =
        static_cast<uint64_t>(currentMould.packTime * 1000.0f);
    if (packTimeMs == 0)
      packTimeMs = INJECT_PACK_STABLE_TIME_MS;

    if (phaseElapsed >= packTimeMs) {
      if (torqueMode) {
        MotorWrapper::setModeAndMove(motor, 1, 6, 0, MODULE_INJECT,
                                     "Inject Torque Stop");
      }
      phase = DONE;
      complete = true;
