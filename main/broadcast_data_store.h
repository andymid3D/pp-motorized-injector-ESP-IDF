...
#define BDS_BUS_VI_HISTORY_SIZE 50
...
public:
  ...
  void storeHeartbeat(uint32_t axisError, uint8_t axisState,
                      uint8_t motorErrorFlag, uint8_t encoderErrorFlag,
                      uint8_t controllerErrorFlag, uint8_t trajectoryDoneFlag,
                      uint64_t timestamp, bool isResponse = false);
  void storeEncoder(float position, float velocity, uint64_t timestamp,
                    bool isResponse = false);
  void storeIq(float iqSetpoint, float iqMeasured, uint64_t timestamp,
               bool isResponse = false);
  void storeBusVI(float busVoltage, float busCurrent, uint64_t timestamp,
                  bool isResponse = false);
  void storeMotorError(uint64_t motorError, uint64_t timestamp,
                       bool isResponse = false);
  void storeEncoderError(uint32_t encoderError, uint64_t timestamp,
                         bool isResponse = false);
  void storeControllerError(uint32_t controllerError, uint64_t timestamp,
                            bool isResponse = false);
...
private:
  ...
  RingBuffer<TimestampedHeartbeat, BDS_HEARTBEAT_HISTORY_SIZE>
      heartbeatHistory_;
  ...
  RingBuffer<TimestampedBusVI, BDS_BUS_VI_HISTORY_SIZE> busVIHistory_;
  RingBuffer<TimestampedMotorError, BDS_MOTOR_ERROR_HISTORY_SIZE>
      motorErrorHistory_;
  RingBuffer<TimestampedEncoderError, BDS_ENCODER_ERROR_HISTORY_SIZE>
      encoderErrorHistory_;
  RingBuffer<TimestampedControllerError, BDS_CONTROLLER_ERROR_HISTORY_SIZE>
      controllerErrorHistory_;
