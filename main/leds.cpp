...
static void apply_brightness(uint32_t color, uint8_t brightness, uint8_t &r,
                             uint8_t &g, uint8_t &b) {
...
static void set_strip_color(ws2812_strip_t *strip, int count, uint32_t color,
                            uint8_t brightness) {
...
static void set_button_colors(uint32_t upper, uint32_t center, uint32_t lower,
                              uint8_t brightness) {
...
void leds_init() {
  if (!ws2812_init(&strip_ring, (gpio_num_t)PIN_LED_RING, ring_pixels,
                   LED_COUNT_RING)) {
    ESP_LOGE(TAG, "Failed to init ring strip");
  }

  if (!ws2812_init(&strip_buttons, (gpio_num_t)PIN_LED_BUTTONS, button_pixels,
                   LED_COUNT_BUTTONS)) {
...
void leds_update(InjectorStates state, bool endOfDay, bool anyButtonPressed,
  ...
  if (!motorPowerEnabled && state != ERROR_STATE) {
    ...
    set_strip_color(&strip_ring, LED_COUNT_RING, colRing, LED_BRIGHT_LOW);
    return;
  }
  ...
  // --- Refill → RED ring (warn user to place nozzle block before Compression)
  // ---
  case REFILL:
    colCenter = GREEN_RGB;
    if (endOfDay) {
  ...
  // --- Purge / AntiDrip / Inject / InjectPack → WHITE ring (illuminate work
  // area) ---
  case PURGE_ZERO:
    colUpper = YELLOW_RGB;
    colCenter = GREEN_RGB;
