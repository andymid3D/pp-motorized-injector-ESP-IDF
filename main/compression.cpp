...
bool update(CanBus &motor) {
  ...
  if (step == TRAVEL_DOWN) {
    if (stateEntry) {
      ...
      MotorWrapper::setMotorLimits(motor, COMPRESS_TRAVEL_VEL_LIMIT,
                                   COMPRESS_TRAVEL_CURRENT, MODULE_COMPRESSION,
                                   "Compress Travel");
      if (!MotorWrapper::setModeAndMove(
              motor, 1, 6, commonParams.compressTravelTorque,
              MODULE_COMPRESSION, "Compress Travel Down Torque")) {
        return false;
      }
      lastCommandTime = time_utils::micros();
      stepTimer = time_utils::micros();
    ...
    // Grace period: ignore stall/torque for first 500ms of travel
    // Grace period: ignore stall/torque for first 500ms of travel
    if (travelElapsed > 500000ULL) {
      if (stallDetected || torqueExceeded) {
        MotorWrapper::setModeAndMove(motor, 1, 6, 0, MODULE_COMPRESSION,
                                     "Compress Stop");
        complete = true;
        step = DONE;
        return true;
