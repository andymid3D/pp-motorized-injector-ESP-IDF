...
bool update(CanBus &motor, bool buttonUp, bool buttonDown,
            bool buttonCenterReleased) {
  ...
  if (!preRetractDone) {
    ...
    if (!preRetractCommanded) {
      ...
      MotorWrapper::setMotorLimits(motor, REFILL_CONTROLLER_VEL_LIMIT,
                                   REFILL_CURRENT_LIMIT, MODULE_PURGE_ZERO,
                                   "Purge PreRetract");
      MotorWrapper::setTrapTrajParams(
          motor, REFILL_TRAP_VEL_LIMIT, commonParams.trapTrajAccelDecel,
          commonParams.trapTrajAccelDecel, MODULE_PURGE_ZERO,
          "Purge PreRetract Traj");
      if (!MotorWrapper::setModeAndMove(motor, 3, 5, preRetractTarget,
                                        MODULE_PURGE_ZERO,
                                        "Purge PreRetract")) {
        return false;
      }
    ...
    uint64_t elapsed = now - lastCommandTime;
    if (elapsed > 500000ULL && ((trajectoryComplete && motorStopped) ||
                                (targetReached && motorStopped))) {
      preRetractDone = true;
      stateEntry = true;
    } else {
      return false;
  ...
  if (stateEntry) {
    ...
    if (upLimit > purgeVelLimit)
      purgeVelLimit = upLimit;
    if (downLimit > purgeVelLimit)
      purgeVelLimit = downLimit;
    MotorWrapper::setMotorLimits(motor, purgeVelLimit,
                                 commonParams.purgeCurrentLimit,
                                 MODULE_PURGE_ZERO, "PurgeZero");
    lastCommandTime = now;
    stateEntry = false;
  }
  ...
  if (currentButtonState != lastButtonState) {
    if (currentButtonState == 1) {
      MotorWrapper::setModeAndMove(motor, 2, 1, commonParams.purgeVelUp,
                                   MODULE_PURGE_ZERO, "Purge Up");
    } else if (currentButtonState == 2) {
      MotorWrapper::setModeAndMove(motor, 2, 1, commonParams.purgeVelDown,
                                   MODULE_PURGE_ZERO, "Purge Down");
    } else {
